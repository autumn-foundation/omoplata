...
| `omo reconcile <id...> [--queue Q]` | Fold several submissions into one merged tree against a queue's landed base (their shared ancestor), carrying same-definition conflicts as **values** (§5.4) instead of refusing — where git rejects a non-fast-forward, omoplata merges (ADR-0010 Phase 3). Disjoint definitions of one file combine; the result is written to a `reconciled/<queue>` head you can `omo switch` onto. Exit 0 clean, 2 if carrying conflict values; a strict queue refuses to keep them. | `omo reconcile sub-a sub-b` |
