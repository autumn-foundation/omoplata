...
| Command | Description | Example |
|---------|-------------|---------|
| `omo remote add <name> <target> [--token T]` | Register another omoplata repository as a named remote — a filesystem path (local-path transport) or an `http://host:port` URL of an `omo serve` daemon (networked transport). `--token` supplies the bearer token for an authenticated daemon. | `omo remote add origin http://host:9000 --token T` |
| `omo remote list` / `omo remote remove <name>` | List or drop registered remotes. | `omo remote list` |
| `omo serve [--addr A] [--token T]` | Serve a repository as a networked **landing authority** over HTTP (ADR-0010): clients `fetch`/`push` against `http://<addr>`. `--token` (or `OMO_TOKEN`) requires `Authorization: Bearer <T>` on every request; without it the server is open (loopback / trusted network). TLS is terminated at a fronting proxy/tunnel, not built in. `--addr 127.0.0.1:0` picks a free port. | `omo serve --addr 127.0.0.1:9000 --token T` |
| `omo fetch <remote>` | Replicate a remote's landed (`public/*`) state (over a path or `http://`): copy the object closure into the local store and record each tip under `remotes/<name>/*` via the op log. Idempotent (content-addressed); private `ws/*` refs are not fetched. Then `omo switch <name>/<change>` drops onto a teammate's remote-landed work — no git round-trip. | `omo fetch origin` |
| `omo push <remote> <id> [--queue Q]` | Land a local submission on a remote through *its* landing policy (the remote is the authority, ADR-0010): replicate the submission's content (over a path or `http://` to an `omo serve` daemon), then run the remote's approval/carried/validator/disjointness gates against the remote's landed state and land under its lock — or refuse with the reason. Content is re-validated by the remote, not trusted. | `omo push origin sub-101 --queue release-1.2` |
| `omo reconcile <id...> [--queue Q]` | Reconcile *without* landing: fold submissions into one merged tree against **the trunk each change was authored on** (recovered from the op log — correct even for a change built on an older trunk), carrying same-definition conflicts as **values** (§5.4) instead of refusing, and write the `reconciled/<queue>` head you can `omo switch` onto (ADR-0010 Phase 3). `omo land`/`omo push` do this automatically. Exit 0 clean, 2 carrying values; a strict queue refuses to keep them. | `omo reconcile sub-a sub-b` |
