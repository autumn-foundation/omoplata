...
| Command | Description | Example |
|---------|-------------|---------|
| `omo remote add <name> <target>` | Register another omoplata repository as a named remote — a filesystem path (local-path transport) or an `http://host:port` URL of an `omo serve` daemon (networked transport). | `omo remote add origin http://host:9000` |
| `omo remote list` / `omo remote remove <name>` | List or drop registered remotes. | `omo remote list` |
| `omo serve [--addr A]` | Serve a repository as a networked **landing authority** over HTTP (ADR-0010): clients `fetch`/`push` against `http://<addr>`. HTTP only, no TLS/auth — loopback or trusted network. `--addr 127.0.0.1:0` picks a free port. | `omo serve --addr 127.0.0.1:9000` |
