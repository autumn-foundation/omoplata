...
| `omo serve [--addr A] [--repo DIR]` | Serve a repository as a networked landing authority over HTTP (ADR-0010): clients `omo fetch http://<addr>` and `omo push http://<addr> <id>`. HTTP only, no TLS/auth — loopback or trusted network. `--addr 127.0.0.1:0` picks a free port (printed on startup). Runs until interrupted. |
