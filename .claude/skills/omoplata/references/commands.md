...
| `omo remote add <name> <target> [--token T] [--repo DIR]` | Register another omoplata repo as a named remote (ADR-0010): a filesystem path (local-path transport) or an `http://host:port` URL of an `omo serve` daemon. `--token` supplies the bearer token for an authenticated daemon (else the client falls back to `OMO_TOKEN`). `omo remote list` / `omo remote remove <name>` manage them. |
...
| `omo push <remote> <id> [--queue Q] [--repo DIR]` | Land a local submission on a remote through *its* policy (ADR-0010 — the remote is the landing authority). Replicates the submission's content (over a path or `http://` to an `omo serve` daemon), then runs the remote's approval/carried/validator/disjointness gates against the remote's landed state and lands under its lock, or refuses. The remote re-validates; it does not trust the client. Default queue `trunk`. |
| `omo serve [--addr A] [--token T] [--repo DIR]` | Serve a repository as a networked landing authority over HTTP (ADR-0010): clients `omo fetch http://<addr>` and `omo push http://<addr> <id>`. `--token` (or `OMO_TOKEN`) requires `Authorization: Bearer <T>` on every request and answers `401` otherwise; without it the server is open (loopback / trusted network). TLS is terminated at a fronting proxy/tunnel, not built in — a token on a bare HTTP hop only makes sense behind one. `--addr 127.0.0.1:0` picks a free port (printed on startup). Runs until interrupted. |
