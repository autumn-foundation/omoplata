...
1. **Merge correctness is an algebraic property, and algebraic properties can be proven.** Git's merge is a heuristic over text with no soundness story. Omoplata puts a small, Verus-verified merge kernel at the center of the system and admits no merge result that does not carry a machine-checked commutation witness. The top-level guarantee: _no silent wrong answers_ — every accepted merge is checked; everything else degrades to an honest, first-class conflict.

2. **The unit of version control is the definition, not the file.** Files are a storage encoding. Identity, merging, semantic search, and agent work-assignment all operate at the level of durable definitions (functions, types, modules) tracked through renames, moves, and refactors. This is the Stoic distinction of _krasis_ vs. _synchysis_: git performs _synchysis_ — fusion that destroys the identity of the components; Omoplata performs _krasis_ — total blending in which each component retains its identity and can be recovered.
...
Omoplata is designed for a development environment where most commits are authored by autonomous agents operating concurrently, review discipline is the binding constraint, and conflict _avoidance_ (detecting convergent work before it collides) is as valuable as conflict resolution.
...
**P5 — Two-tier identity.** Commits are content-addressed (SHA-256, hash-agile format). _Changes_ carry stable IDs that survive rebase and amend, linked by supersession edges (Mercurial obsolescence, done properly). Phases (draft/public) formalize what is safe to rewrite.
...
flowchart TB
    ...
    subgraph Surface["Surface"]
        WC[Working model<br/>stacks, workspaces, WC-as-commit]
        SUB[Submissions<br/>interdiffs, carry-forward, comments]
        MQ[Merge queue<br/>Tier-0 batching, phase transition]
        RV[Revset query engine]
        GI[Git interop<br/>object format + wire protocol<br/>bookmarks as shadow]
        CLI[CLI]
    end
...
_Tier 0 — Disjoint support._ Each patch carries a _support set_: the definition IDs (plus file-region intervals for opaque content) it touches. Patches with disjoint support commute by lemma I10; the check is a set intersection against the definition graph — no diff-level work, no cache. At fleet concurrency this screens out the overwhelming majority of pairs.

_Tier 1 — Commutation._ Derived patches that provably commute are merged by the kernel directly. No heuristics, no language awareness, proof-backed. Resolves everything genuinely independent (the majority of fleet-concurrent changes).

_Tier 2 — Structural._ Surviving conflicts go to the per-language structural driver: parse base and both sides to concrete syntax trees, match nodes (GumTree-family: top-down identical-subtree matching, bottom-up container recovery, strengthened by definition-identity anchors from P6), propose a merged tree. Trivia is owned by tokens under a Roslyn-style deterministic policy (same-line trailing comment binds to the preceding token; otherwise a comment leads the next named node; blank-line runs split at the first blank), applied identically at parse time on all three versions — so moved nodes carry their trivia by construction and ownership is never arbitrated at merge time. Where the repo declares a canonical formatter (v1: rustfmt), whitespace trivia is regenerable and exits the problem entirely; byte-faithfulness for whitespace is formatter-canonical. Pragma comments (per-language recognizer defaults, repo-extensible config) are _bound trivia_: hard-anchored to their node, and any contest between the pragma and its anchor across sides escalates to Tier-3 by rule, never policy — because a detached pragma compiles and passes tests, making it invisible to dynamic validation (P9). Kernel admission for Tier-2 proposals checks tree equality _and_ trivia conservation (I11). Acceptance is provisional pending dynamic validation (P9). Kills the false-conflict class: reformatting, moves, renames, reorderings.

_Tier 3 — Semantic conflict._ What survives Tiers 1–2, or fails dynamic validation, is presented as a semantic conflict: both sides' definition-level intent, provenance (which agent, which spec), and embedding-derived context — not `<<<<<<<` soup. Resolvable by human or arbiter agent, asynchronously (P3).
...
Every node carries typed embeddings (AletheiaDB native): diff content, change description/spec text, touched-definition signatures — independently indexed. Capabilities: semantic bisect and archaeology (vector query joined to the definition graph), review routing (embed-nearest past changes → their reviewer), duplicate-work detection (embedding-adjacent in-flight changes from different agents flagged _before_ textual collision — conflict avoidance, the cheapest tier of all), and provenance queries (agent, spec, supersession lineage as graph edges under revsets).

### 5.8 Query layer

Revsets (Mercurial lineage, jj dialect) extended over changes, definitions, conflicts, operations, and embedding predicates — plus workflow verbs: `stack()`, `open()`, `mine(<agent>)`, `trunk()`, `queued()`. Target expressiveness: `definitions(touching: auth::*) & superseded(after: review-approved) & agent(eris)` as one expression.

### 5.9 Working and branching model

Branches are not a primary object. The primary objects are **changes and stacks**; heads are anonymous, and bookmarks are **purely interop shadow** — read-only reflections of git branches at the P8 boundary, never locally mutable names (local mutable branch pointers would reintroduce the pointer religion the change graph exists to escape). The draft-phase region of the change graph is the workspace, navigated by revsets.

A **stack** is a linear chain of draft changes. The Sapling verb set is kept verbatim (earned muscle memory), with each verb upgraded by the algebra:

- **`absorb`** routes working-copy hunks to the stack change that last modified the touched definition — a definition-graph lookup, not a line-range heuristic. Ambiguous routing goes interactive, never guesses; the whole absorb is kernel-checked (the amended stack must reproduce the working tree exactly) so it structurally cannot guess wrong silently.
- **`reorder`** is a decidable query: adjacent changes with disjoint support (Tier 0) swap provably; commuting changes (I5 check) swap safely; neither — the reorder still succeeds but materializes an honest conflict value (P3). Stack surgery stops being rebase roulette.
- **Amend-anywhere** auto-rebases descendants (P4), records supersession (P5); non-overlapping downstream propagation is frictionless by proof.
- **`split`/`fold`** emit declared-intent objects (§5.5 Tier A) so definition identity flows through stack surgery rather than being re-inferred after it.

**Workspaces, one per agent:** multiple working copies over one repo, each an auto-snapshotted commit — always recorded, crash-recoverable, queryable. Duplicate-work detection (§5.7) reads live workspaces, catching convergence before either agent commits. Eliminates multi-agent lock contention currently papered over with git worktrees.

**Megamerge** (documented pattern, not a primitive): park a workspace on the merge of N in-flight stacks — conflicts-as-values makes this always well-defined — and continuously test integration of unfinished work. Fleet form: a standing integration workspace (Sentinel-owned) surfacing cross-stack breakage via P9 before landing order matters.

### 5.10 Review and landing: submissions

The review unit is a **submission**: a typed object referencing an evaluated revset of **change IDs, not commits** — so amends and rebases flow into the same submission via supersession instead of orphaning the review, and the conversation has continuous identity across a change's life. Partial-stack submissions are legal (any revset); unsubmitted parents become **implicit dependency edges, enforced by the queue**.

_Interdiffs subtract the rebase._ The op log records each revision's base, so the review-relevant delta between revisions is `diff(rebase(revN, B2), revN+1)` — the author's edits with trunk motion factored out, computed by the same deterministic diff as everything else. The two time axes distinguishing "author changed it" from "the world changed under it" is exactly what bi-temporality is for.

_Approval carry-forward with a commutation certificate._ When a reviewed change is rebased, Tier-0/1 answers provably whether the rebase intersects the change's support. Disjoint support or checked commutation ⇒ the reviewed content is unchanged as a matter of proof, and the approval persists with an I5-backed certificate attached — not a forge heuristic. **Policy-configurable per repo, certificate-auto as the default** (a security-sensitive repo may demand human eyes on every rebase regardless of proof; policy belongs to the repo, not the tool). Re-review is requested only when the rebase genuinely touched reviewed content, and the interdiff shows exactly the intersection. For a fleet generating constant rebases, this is the largest review-throughput lever in the system.

_Comments anchor to definitions, not lines_ — a definition ID plus node path, re-anchored through revisions by the same tree matching as everything else, following code through renames, moves, and reformatting. Ambiguous re-anchoring degrades to preserved-but-unanchored, never silent drop. Comment threads carry embeddings (§5.7), making "has this concern been raised on this definition before" a query — review memory attached to definition history. **v1 scope: comments through the CLI**; approvals are bi-temporal assertions (revocable, supersedable, auditable per §5.6).

_Landing is the merge queue_ (**v1**). `land` enqueues; the queue gates on approvals-per-policy plus dynamic validation (P9); landing is the draft→public phase transition (P5). Batching by Tier 0: pairwise-disjoint changes batch, test as one, land in parallel (I3′ licenses order-independence); overlapping changes serialize with commutation checks; batch failures bisect by support set rather than blind halving. Within a stack, a change that provably commutes with its unlanded parents may land out of order, certificate attached — stack order becomes authoring convenience rather than landing constraint wherever the algebra licenses it. GitHub's merge queue is a forge-side reimplementation of what the VCS should own, minus the theorem that makes batching sound.

_Forge boundary_ (per Q5): each change exports as a PR branch, stacks as chained PRs, supersession as force-push — lossy outbound, faithful inbound; native review richness exists only inside, where the N=1 network lives.
...
- **I2 Merge commutativity — holds by construction, no proof obligation:** merge is defined over a hash-sorted _set_ of sides; canonical order is imposed internally before any driver sees input, so `merge(a,b)` and `merge(b,a)` are the same computation. (Determinism-by-fiat, same move as I1a.)
...
| 3   | `omoplata-identity` | Change graph, supersession, phases, definition graph, submission/approval objects — I6 | Verified (graph invariants; easy proofs)                      |
| 4   | `omoplata-work`     | Working model: WC-as-commit, stacks, workspaces, auto-rebase, op log, merge queue — I7 | Glue around verified core                                     |
...
**v1 in-scope:** verified merge algebra; conflicts-as-values working model with stacks and per-agent workspaces (bookmarks purely interop-shadow); two-tier identity (change + definition); submissions with interdiffs, certificate-based approval carry-forward (policy-configurable, certificate-auto default), and CLI comments; the merge queue with Tier-0 batching and out-of-order stack landing; bi-temporal op log with revsets; embeddings + duplicate-work detection; git interop with round-trip gate; Tier-2 structural merge for **Rust only** (one grammar, dogfooded on the Autumn stack), Mergiraf as the fallback driver for everything else.
...
3. **M2:** `omoplata-identity` + `omoplata-work`: init/commit/rebase/undo on a real repo; conflicts ride through rebases; stacks and per-agent workspaces operational; submissions with interdiffs and approval objects; merge queue landing changes **serialized** (batching deferred to M4's validation loop).
...
5. **M4:** Rust Tier-2 driver behind the kernel check; Mergiraf fallback wired; Sentinel dynamic-validation loop closed — which activates queue **batching** (Tier-0 batches, support-set bisection on failure, out-of-order stack landing) and certificate carry-forward end to end.
...
**Omoplata**: Portuguese for shoulder blade aksjdfkajskdfjasdf, from Greek ὠμοπλάτη (_ōmos_, shoulder + _platē_, blade) — the pantheon survives on a technicality. In Brazilian jiu-jitsu, the omoplata is a shoulder lock won by position and leverage rather than force: the arm is isolated, rotation is controlled, and resistance becomes submission. Which is the merge philosophy exactly — conflicts are not smashed, they are held as values and rotated until they resolve. The system's conceptual vocabulary retains the Stoic _krasis_/_synchysis_ distinction (§1): git performs synchysis, fusion destroying component identity; Omoplata performs krasis, blending in which every component remains recoverable. (Krasis was the working name; it survives as the name of the property, which is arguably where it belonged.) Namespace sweep (crates.io, GitHub, USPTO, domains) deliberately not yet run — skunkworks doesn't need a trademark.

---

_ADR-0001 candidate: "Snapshot-primary, patch-derived representation" (P2, Q1). ADR-0002 candidate: "LCF kernel/proposer admission" (P1, I8)._
