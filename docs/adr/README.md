...
| [ADR-0010](0010-distributed-omoplata.md) | **Distributed omoplata**: replicate the content-addressed store + selective ref sync, with a remote as the **landing authority** (the trust boundary is landing, not transport). Phase 1 (shipped) is read-replication — `omo remote`/`omo fetch` + remote-tracking refs, making `omo switch` work across repos; Phases 2–3 (remote submit/land, concurrent-push reconciliation through the kernel) are documented future. |
