...
| [ADR-0010](0010-distributed-omoplata.md) | **Distributed omoplata**: replicate the content-addressed store + selective ref sync, with a remote as the **landing authority** (the trust boundary is landing, not transport). Phase 1 (shipped): read-replication — `omo remote`/`omo fetch` + remote-tracking refs make `omo switch` work across repos. Phase 2 (shipped): `omo push` lands a submission on a remote through *its* policy, re-validated against *its* trunk under *its* lock. Phase 3 (concurrent-push reconciliation through the kernel) is documented future. |
